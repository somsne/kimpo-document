...
- 三联单（如"存根 / 客户 / 财务"）：同样的手法，把 frame 组从两组扩到三组（或改用 repeat.copies 自动复制，见 §5.4）；
...
### 5.4 两种写法：显式摆联 vs repeat.copies（2026-07-15 真机修正）

本演示采用**显式摆联**写法：两联的全部 frame 都在 `frames[]` 里摆出来，**不需要 `repeat`**——所见即所得，语义最直白。早期文稿曾同时带一段 `repeat.copies` 声明，真机校验证实那是非法写法（凡带 `repeat` 就必须给 `frameSetPerItem` 非空数组），已删除。

若想用 `repeat.kind: "copies"` 自动复制（联数多、不想手摆时），正确姿势是：只摆**一联**的 frame 组（rect 相对格子左上角），用 `frameSetPerItem` 引用这组 frame，`rows: 3, cols: 1` 即自动纵向铺 3 联；联名等"每联不同的文字"这种差异化诉求则回到显式摆联写法。copies 不绑定 `dataSource`，每联数据都是整张单据本身。
